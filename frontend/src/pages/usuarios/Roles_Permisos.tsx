...
export default function GestionRolesPermisos() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const url = type === "crear" ? "/api/roles" : `/api/roles/${rol?.id}`;
  ...
  const handleGuardarPermisos = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/roles/${rol.id}/permisos`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-600">
                  {rol.cantidadPermisos || 0} permisos
                </span>
              </div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg space-y-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-gray-900">
                        {dialogState.rol.cantidadPermisos || 0} permisos
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 mb-1">Fecha de Creación</p>
                      <p className="text-sm text-gray-900">
                        {new Date(dialogState.rol.creadoEn).toLocaleDateString("es-ES", {
                          year: "numeric",
                          month: "long",
                          day: "numeric",
                        })}
