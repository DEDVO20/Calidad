...
export default function EditarUsuario() {
    ...
    useEffect(() => {
        const loadData = async () => {
            await Promise.all([fetchAreas(), fetchRoles()]);
            if (id) {
                await fetchUsuario(id);
            }
        };
        loadData();
    }, [id]);
    ...
    const fetchUsuario = async (userId: string) => {
        ...
        try {
            ...
            setFormData({
                ...
                contrasena: "", // No cargamos la contraseña
                confirmarContrasena: "",
                areaId: user.areaId || "",
                activo: user.activo,
            });

            // Cargar roles asignados si vienen en la respuesta, sino buscar aparte
            // Asumimos que el backend podría devolver roles en el objeto usuario o necesitamos otro endpoint
            // Por ahora, asumimos que user.roles trae los roles asignados
            if (user.roles && Array.isArray(user.roles)) {
                setSelectedRoleIds(user.roles.map((r: any) => r.id));
            } else {
                // Fallback: fetch roles del usuario (si el endpoint principal no los trae)
                // const rolesResponse = await fetch(`/api/usuarios/${userId}/roles`, ...)
            }
    ...
    const validateForm = (): boolean => {
        ...
        // Validación de contraseña solo si se ingresó algo (opcional en edición)
            ...
            <form onSubmit={handleSubmit}>
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Edite los campos necesarios. Deje la contraseña en blanco si no desea cambiarla.
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                        {/* Información Personbal */}
                        ...
                        <div className="flex gap-3 pt-6 border-t">
                            ...
                            <Button type="button" variant="outline" onClick={() => navigate("/usuarios")} disabled={loading} className="flex-1">
